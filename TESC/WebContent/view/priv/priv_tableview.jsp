...
<%response.setContentType("text/html;charset=UTF-8");request.setCharacterEncoding("UTF-8");%>
	...
	<script type="text/javascript">  
			function submitPrivForm(selectidvar){
				
				/*USE AJAX SEND FORM*/ 
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=editpriv",  
					data:{selectid: selectidvar,
						  autoIndex: $("#priv_autoIndex").val(),  
						  state: $("#priv_state").val(),
						  title: $("#priv_tittle").val(),
						  privLevel: $("#priv_privLevel").val(),
						  ps: $("#priv_ps").val()
					},  
					dataType:"json",  
					cache:false,  
			...
			$(document).ready(function() {  
				$("#flexDb_priv").flexigrid ({  
					//遠端伺服的網址
					url: '/TESC/rolemanager.do?action=getprivtable',  
					//url: '/TESC/test.jsp',
					//回傳的資料類型
					dataType: 'json',  
					//定義欄位資訊
					colModel: [  
						...
						{display: '權限名', name : 'tittle', width : 120, sortable : true, align: 'left'},
						{display: '權限層級數', name : 'privLevel', width : 120, sortable : true, align: 'left'},
			...
			function updateGrid(com, grid){
				if (com=='Add'){
					$("#grideditform").load("/TESC/rolemanager.do?action=loadprivform",{selectid:'-1'},function(){
					...
					if($(".trSelected").length==1){  
						...
						$("#grideditform").load("/TESC/rolemanager.do?action=loadprivform",{selectid: selectidvar},function(){
							$("#grideditform").fadeIn('slow');}
						);  
			...
			function deleteGrid(selectidvar){
				...
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=deletepriv",  
					data:{selectid: selectidvar 
					},  
					dataType:"json",  
					cache:false,  
