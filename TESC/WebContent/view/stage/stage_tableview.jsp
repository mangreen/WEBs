...
<%response.setContentType("text/html;charset=UTF-8");request.setCharacterEncoding("UTF-8");%>
	...
	<script type="text/javascript">  
			...
			function submitForm(selectidvar){
				...
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=editstage",  
			...
			$(document).ready(function() {  
				...
				$("#flexDb_priv").flexigrid ({  
					//遠端伺服的網址
					url: '/TESC/rolemanager.do?action=getstagetable',  
					//回傳的資料類型
					dataType: 'json',  
					//定義欄位資訊
					colModel: [  
						{display: '主鍵', name : 'autoIndex', width : 60, sortable : true, align: 'center'},  
						{display: '階段名', name : 'title', width : 120, sortable : false, align: 'left'}
					],  
			...
			function updateGrid(com, grid){
				if (com=='Add'){
					$("#grideditform").load("/TESC/rolemanager.do?action=loadstageform",{selectid:'-1'},function(){
					...
					if($(".trSelected").length==1){  
						...
						$("#grideditform").load("/TESC/rolemanager.do?action=loadstageform",{selectid: selectidvar},function(){
							$("#grideditform").fadeIn('slow');}
						);  
			...
			function deleteGrid(selectidvar){
				...
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=deletestage",  
					data:{selectid: selectidvar, 
					},  
					dataType:"json",  
					cache:false,  
