...
<%response.setContentType("text/html;charset=UTF-8");request.setCharacterEncoding("UTF-8");%>
	...
	<script type="text/javascript">  
			...
			function submitForm(selectidvar){
				...
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=editdepart",  
					data:{selectid: selectidvar,
						  autoIndex: $("#department_autoIndex").val(),  
						  title: $("#department_title").val(),
						  code: $("#department_code").val(),
						  address: $("#department_address").val(),
						  phone: $("#department_phone").val(),
						  mobile: $("#department_mobile").val()
					},  
					dataType:"json",  
					cache:false,  
			...
			$(document).ready(function() {  
				...
				$("#gridtable").flexigrid ({  
					//遠端伺服的網址
					url: '/TESC/rolemanager.do?action=getdeparttable',  
					//回傳的資料類型
					dataType: 'json',  
					//定義欄位資訊
					colModel: [  
						{display: '主鍵', name : 'autoIndex', width : 60, sortable : true, align: 'center'},  
						{display: '系所名', name : 'title', width : 60, sortable : false, align: 'left'},  
						{display: '校務系統代碼', name : 'code', width : 60, sortable : true, align: 'left'},
						{display: '地址', name : 'address', width : 180, sortable : true, align: 'left'},
						{display: '聯絡電話', name : 'phone', width : 100, sortable : true, align: 'left'},
						{display: '行動電話', name : 'mobile', width : 100, sortable : true, align: 'left'}
			...
			function updateGrid(com, grid){
				if (com=='Add'){
					$("#grideditform").load("/TESC/rolemanager.do?action=loaddepartform",{selectid:'-1'},function(){
					...
					if($(".trSelected").length==1){  
						...
						$("#grideditform").load("/TESC/rolemanager.do?action=loaddepartform",{selectid: selectidvar},function(){
							$("#grideditform").fadeIn('slow');}
						);  
			...
			function deleteGrid(selectidvar){
				...
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=deletedepart",  
					data:{selectid: selectidvar 
					},  
					dataType:"json",  
					cache:false,  
