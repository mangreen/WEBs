...
<%response.setContentType("text/html;charset=UTF-8");request.setCharacterEncoding("UTF-8");%>
	...
	<script type="text/javascript">  
			...
			function submitForm(selectidvar){
				...
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=editconfig",  
					data:{selectid: selectidvar,
						  autoIndex: $("#config_autoIndex").val(),  
						  title: $("#config_title").val(),
						  val: $("#config_val").val(),
						  val2: $("#config_val2").val()
					},  
					dataType:"json",  
					cache:false,  
			...
			$(document).ready(function() {  
				...
				$("#gridtable").flexigrid ({  
					//遠端伺服的網址
					url: '/TESC/rolemanager.do?action=getconfigtable',  
					//回傳的資料類型
					dataType: 'json',  
					//定義欄位資訊
					colModel: [  
						{display: '主鍵', name : 'autoIndex', width : 60, sortable : true, align: 'center'},  
						{display: 'title', name : 'title', width : 60, sortable : false, align: 'left'},  
						{display: 'val', name : 'val', width : 60, sortable : true, align: 'left'},
						{display: 'val2', name : 'val2', width : 60, sortable : true, align: 'left'}, 
					],  
			...
			function updateGrid(com, grid){
				if (com=='Add'){
					$("#grideditform").load("/TESC/rolemanager.do?action=loadconfigform",{selectid:'-1'},function(){
					...
					if($(".trSelected").length==1){  
						...
						$("#grideditform").load("/TESC/rolemanager.do?action=loadconfigform",{selectid: selectidvar},function(){
							$("#grideditform").fadeIn('slow');}
						);  
			...
			function deleteGrid(selectidvar){
				...
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=deleteconfig",  
					data:{selectid: selectidvar 
					},  
					dataType:"json",  
					cache:false,  
