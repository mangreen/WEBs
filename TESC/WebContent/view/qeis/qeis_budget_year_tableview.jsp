...
<%response.setContentType("text/html;charset=UTF-8");request.setCharacterEncoding("UTF-8");%>
	...
	<script type="text/javascript">  
			...
			function submitForm(selectidvar){
				...
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=editqeis",  
					data:{selectid: selectidvar,
						  autoIndex: $("#qeis_budget_year_autoIndex").val(),  
						  budgetYear: $("#qeis_budget_year_budgetYear").val(),
						  currentPrice: $("#qeis_budget_year_currentPrice").val(),
						  capitalPrice: $("#qeis_budget_year_capitalPrice").val()
					},  
					dataType:"json",  
					cache:false,  
			...
			$(document).ready(function() {  
				...
				$("#gridtable").flexigrid ({  
					//遠端伺服的網址
					url: '/TESC/rolemanager.do?action=getqeistable',  
					//回傳的資料類型
					dataType: 'json',  
					//定義欄位資訊
					colModel: [  
						{display: '主鍵', name : 'autoIndex', width : 60, sortable : true, align: 'center'},  
						{display: '年度', name : 'budgetYear', width : 120, sortable : false, align: 'left'},  
						{display: '經常門金額', name : 'currentPrice', width : 185, sortable : true, align: 'left'},
						{display: '資本門金額', name : 'capitalPrice', width : 185, sortable : true, align: 'left'}, 
					],  
			...
			function updateGrid(com, grid){
				if (com=='Add'){
					$("#grideditform").load("/TESC/rolemanager.do?action=loadqeisform",{selectid:'-1'},function(){
					...
					if($(".trSelected").length==1){  
						...
						$("#grideditform").load("/TESC/rolemanager.do?action=loadqeisform",{selectid: selectidvar},function(){
							$("#grideditform").fadeIn('slow');}
						);  
			...
			function deleteGrid(selectidvar){
				...
				$.ajax({  
					type:"POST",  
					url:"/TESC/rolemanager.do?action=deleteqeis",  
					data:{selectid: selectidvar 
					},  
					dataType:"json",  
					cache:false,  
