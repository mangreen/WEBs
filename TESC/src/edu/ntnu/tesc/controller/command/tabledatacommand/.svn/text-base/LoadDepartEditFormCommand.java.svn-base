...
public class LoadDepartEditFormCommand implements ICommand {
	...
	public IModelView execute() {
		...
		try{
			...
			if(i>=0){
				model = departmentDAO.getDepartment(i);
		...
		modelView.setView(viewList.getDeparteditformPage());
		modelView.setModule(model);
		modelView.setModelName("department");
		return modelView;
	}
