...
public class DeletePrivCommand implements ICommand {
	...
	public IModelView execute() {
		...
		if(selectid != null && !selectid.equals("null") && !selectid.equals("-1") ){
			try{
				...
				int i = privlegeDAO.delPrivilege(seletedid);
				LogManager.getLogger(this.getClass()).info("delete privilege id : "+i);
