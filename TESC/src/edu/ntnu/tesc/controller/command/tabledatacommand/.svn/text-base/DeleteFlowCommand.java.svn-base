...
public class DeleteFlowCommand implements ICommand {
	...
	public IModelView execute() {
		...
		if(selectid != null && !selectid.equals("null") && !selectid.equals("-1") ){
			try{
				...
				int i = flowModuleDAO.delFlowModule(seletedid);
				LogManager.getLogger(this.getClass()).info("delete flow module id : "+i);
